...
public class OnSubscribeRefreshSelectTest {
    ...
    @Test(timeout = 1000)
    public void test2() {
        checkEquals(asList(1, 2, 3, 3, 4, 4, 5, 6, 6, 7), just(1, 2, 5, 6), just(3, 4, 4, 7),
                just(3, 6));
    }
    ...
    public void test3() {
        checkEquals(Collections.<Integer> emptyList(), Observable.<Integer> empty(),
    ...
    @Test(timeout = 1000)
    public void test5() {
        checkEquals(asList(1, 2), just(1).onBackpressureBuffer(), just(2).onBackpressureBuffer());
    }

    @SuppressWarnings("unchecked")
    @Test(timeout = 1000)
    public void test6() {
        checkEquals(asList(1, 2, 3, 4), just(1, 2, 4), just(3).onBackpressureBuffer());
    }

    @SuppressWarnings("unchecked")
    @Test(timeout = 1000)
    public void test7() {
        checkEquals(asList(1, 2, 4, 5, 7, 8, 9), just(1, 2, 4), Observable.<Integer> empty(),
                just(5, 7), just(8, 9));
    }
    ...
    public void testRecursion() {
        ...
        System.out.println(ones.compose(f).compose(f).compose(f).compose(f).compose(f).compose(f)
                .compose(f).count().toBlocking().single());

        int sum = ones.map(Util.<Integer> nested())
                .reduce(new Func2<Observable<Integer>, Observable<Integer>, Observable<Integer>>() {

                    @Override
                    public Observable<Integer> call(Observable<Integer> a, Observable<Integer> b) {
                        return a.concatWith(b).toList().map(total);
                    }
                }).flatMap(Functions.<Observable<Integer>> identity()).single().toBlocking()
                .single();
