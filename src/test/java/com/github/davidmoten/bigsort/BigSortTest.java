...
public class BigSortTest extends TestCase {
	...
	public static TestSuite suite() {
		...
		return TestingHelper
				.function(sorter(2, 2, Schedulers.immediate()))
	...
	private void performSerializableTest(int n, int maxToSortPerThread,
			int maxTempFiles) {
		Observable<Integer> source = Observable.range(1, n).map(i -> n - i + 1);
		BigSort.sort(source, maxToSortPerThread, maxTempFiles,
	...
	private static void performTest(int size, int maxToSortPerThread,
			int maxTempFiles) {
		log.info("sorting " + size + " values using maxToSort="
				+ maxToSortPerThread + " and maxTempFiles=" + maxTempFiles);
		final int n = size;// passes on 127!!
		// source is n, n-1, .., 0
		Observable<Integer> source = createDescendingRange(n);
		List<Integer> range = Observable.range(1, n).toList().toBlocking()
				.single();
		long t = System.currentTimeMillis();
		assertEquals(
				range,
				sorter(maxToSortPerThread, maxTempFiles, Schedulers.immediate())
						.call(source).toList().toBlocking().single());
		log.info("time to sort using immediate() = "
				+ ((System.currentTimeMillis() - t) / 1000.0) + "s");
		t = System.currentTimeMillis();
		assertEquals(
				range,
				sorter(maxToSortPerThread, maxTempFiles,
						Schedulers.computation()).call(source).toList()
						.toBlocking().single());
	...
	private static Func1<Observable<Integer>, Observable<Integer>> sorter(
		...
		return source -> {
			...
			Func0<File> resourceFactory = BigSort
					.createFileResourceFactory(Optional.of("target"));
			Action1<File> resourceDisposer = BigSort
					.createFileResourceDisposer();
